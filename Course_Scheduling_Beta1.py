...
def genAlg(numCourses,courses,popSize,times,rooms,maxIters,isElite):
	...
	while (i < maxIters) and ((float(max(fit))/(numCourses*7)) < 0.9):
		...
		for j in range(0,len(toPull)):
			#return the index of top element in top pop
...
def printCourse(course):
  ...
  for i in range(0,len(course),7):
    ...
    print ("Requested Time: " + str(course[i+4]))
    print ("Requested Room: " + str(course[i+5]))
...
def temp():
	...
	initial = []
	file = open("C:\\Users\\ABDU\\Documents\\consts.txt",'w')

	for i in range(0,25):
		file.write("Professor: " +profs[random.randrange(0,8)]+"\n")
		file.write("Course: " +courses[random.randrange(0,4)]+"\n")
		file.write("Room: " +rooms[random.randrange(0,5)]+"\n")	
		file.write(str(20)+"\n")		
		file.write("Time: " +times[random.randrange(0,8)]+"\n")
